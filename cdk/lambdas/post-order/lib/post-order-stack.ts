...
import * as policyJson from './roles/post-order-role-policies.json';

export class PostOrderStack extends Stack {
  constructor(scope: Construct, id: string) {
    ...
    let lambdaFunctionName = `post-order-${process.env.AWS_DEPLOY_REGION}`;
    let roleName = `post-order-role-${process.env.AWS_DEPLOY_REGION}`;
